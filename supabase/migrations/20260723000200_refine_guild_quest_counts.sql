...
as $$
  ...
  where ranked.rank <= case when target_kind = 'side' then 4 else 2 end;
$$;

create or replace function loro_private.guild_candidate_ids_for_board(
  target_kind text,
  target_period_key date,
  stored_locked_ids text[]
)
returns text[]
language sql
stable
...
as $$
  select case
    when cardinality(coalesce(stored_locked_ids, '{}')) >= case when target_kind = 'side' then 4 else 2 end then
      coalesce(stored_locked_ids, '{}')
    else
      array_cat(
        coalesce(stored_locked_ids, '{}'),
        (
          select coalesce(array_agg(candidate.id order by candidate.first_position), '{}')
          from (
            select candidate_id as id, min(position) as first_position
            from unnest(
              loro_private.guild_candidate_ids(target_kind, target_period_key)
            ) with ordinality as candidate_ids(candidate_id, position)
            where not (candidate_id = any(coalesce(stored_locked_ids, '{}')))
            group by candidate_id
            order by first_position
            limit (case when target_kind = 'side' then 4 else 2 end - cardinality(coalesce(stored_locked_ids, '{}')))
          ) candidate
        )
      )
  end;
...
update public.guild_quest_boards board
set side_candidate_ids = loro_private.guild_candidate_ids_for_board(
      'side',
      board.side_period_key,
      board.side_locked_ids
    ),
    main_candidate_ids = loro_private.guild_candidate_ids_for_board(
      'main',
      board.main_period_key,
      board.main_locked_ids
    );
