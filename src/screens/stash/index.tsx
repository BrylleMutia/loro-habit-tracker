...
export function StashScreen({ onDailyCheckInPress }: StashScreenProps) {
  const { inventory } = useGameInventory();
  const { profile } = useGameProfile();
  const { isOnline, mutationInFlight, syncError } = useGameSync();
  ...
  const allGroupedItems = useMemo(
    () =>
      isDetailsReady
        ...
        : [],
    [inventory.items, isDetailsReady, profile.equippedItemIds, sortDirection, sortKey]
  );
  const groupedItems = useMemo(
    () =>
      slotFilter === "all"
        ? allGroupedItems
        : allGroupedItems.filter((stack) => stack.representative.slotId === slotFilter),
    [allGroupedItems, slotFilter]
  );
    ...
    >
      <View className="self-center" style={{ width: contentWidth }}>
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="font-display text-2xl font-black text-content">Stash</Text>
        ...
        {isDetailsReady ? (
          ...
          <View className="mt-4 rounded-card border border-line bg-surface-card p-4" style={shadows.card}>
            <Text className="text-sm font-black text-content">Loading trail gear…</Text>
            <Text className="mt-1 text-xs font-semibold text-content-muted">
              Loadout, filters, and inventory are getting the stash ready.
            </Text>
          </View>
        )}
      </View>
